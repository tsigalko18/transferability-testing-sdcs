...
from visualodometry.own_models_with_dropout import DAVE2_MC, get_own_model_with_dropout, CHAUFFEUR_MC, EPOCH_MC
...
def get_model_by_type(model_type, cfg):
    ...
    if model_type in [DAVE2_MC, CHAUFFEUR_MC, EPOCH_MC]:
        input_size = (
            int(input_shape[0] - roi_crop[0] - roi_crop[1]),
            int(input_shape[1]),
            int(input_shape[2])
        )
        return get_own_model_with_dropout(model_type, input_size)()
